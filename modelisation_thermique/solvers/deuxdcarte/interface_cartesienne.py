...
from solvers.deuxdcarte.modelisation_2D import *
#from affichage.interface import menu
...
def create_main_window():
    ...
    checkbox_type0 = tk.Checkbutton(
        root, text="Constant", variable=var_type, onvalue=0, offvalue=1)
    checkbox_type0.pack()

    checkbox_type1 = tk.Checkbutton(
        root, text="Linéaire", variable=var_type, onvalue=1, offvalue=2)
    checkbox_type1.pack()

    checkbox_type2 = tk.Checkbutton(
        root, text="Rampe", variable=var_type, onvalue=2, offvalue=3)
    checkbox_type2.pack()

    # Bouton pour ouvrir la fenêtre de configuration des paramètres
    config_button = tk.Button(
        root, text="Configurer les paramètres", command=open_config_window)
    config_button.pack()
    ...
    def passer_anim():
        root.destroy()
        fig, anim = animcarte2D()
        affichage_2Dcarte(fig, anim)
    ...
    def rajoute_source():
        ...
        print(data["sources"])
        print("Ajout d'une source en", (x0, y0),
              "de rayon", r, "cm et puissance", Pv, "J")
    ...
    Pv_s.pack()
    ajtSrc = tk.Button(root, text="Ajouter cette source",
                       command=rajoute_source)
    ajtSrc.pack()
    PopSrc = tk.Button(
        root, text="Supprimer la dernière source ajoutée", command=supprime_source)
    PopSrc.pack()

    # Bouton pour lancer l'animation
    launch_button = tk.Button(root, text="Lancer", command=passer_anim)
    launch_button.pack()
...
def affichage_2Dcarte(fig, anim):
    ...
    apply_button = ttk.Button(btn_frame,
                              text="New Params",
    ...
    apply_button.grid(column=5, row=0, pady=10, padx=30)
