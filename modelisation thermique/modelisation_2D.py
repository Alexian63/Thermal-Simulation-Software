...
def launch():
    ...
    for i in range (len(data["sources"])):
        ajoute_source(data["sources"][i][0],data["sources"][i][1],data["sources"][i][2],data["sources"][i][3])
        ...
        for t in range(1, nt):
            for i in range(1, Ny - 1):
                for j in range(1, Nx - 1):
                    T[t, i, j] = T[t-1,i,j] + data["S"][i,j]*dt + data["K"]*dt*((T[t-1,i,j+1]-2*T[t-1,i,j]+T[t-1,i,j-1])/(dx**2)+(T[t-1,i+1,j]-2*T[t-1,i,j]+T[t-1,i-1,j])/(dy**2))
        return T
    ...
    anim = FuncAnimation(fig,animate,frames = nt//10,interval = 1e-10, repeat = True)
